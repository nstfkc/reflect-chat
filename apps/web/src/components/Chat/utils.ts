...
export function groupItemsByCreatedAt<T extends MessageV1>(
  items: T[]
): Record<string, T[]> {
...
function uniqueById<T extends MessageV1>(messages: T[]): T[] {
  let out: T[] = [];
  let cache: string[] = [];
...
export function groupMessagesInTheSameMinute<T extends MessageV1>(
  messages: T[]
): Record<string, T[]> {
